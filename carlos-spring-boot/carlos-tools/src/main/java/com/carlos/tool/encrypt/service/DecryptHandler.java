...
import cn.hutool.db.*;
import com.carlos.tool.encrypt.config.DatabaseInfo;
import com.carlos.tool.encrypt.config.ToolInfo;
import com.carlos.tool.encrypt.config.ToolInfo.Encrypt;
 ...
 */
@Slf4j
public class DecryptHandler implements DataHandler {
    ...
    public DecryptHandler(DatabaseService databaseService, ToolInfo toolInfo) {
    ...
    public void handle() {
        ...
        log.info(String.format("解密字段为：%s", StrUtil.join(StrUtil.COMMA, fields)));
            ...
            for (Entity entity : pageResult) {
                ...
                for (String field : fields) {
                    ...
                    Optional.ofNullable(value).ifPresent(i -> {
                        ...
                        String decryptStr;
                        try {
                            decryptStr = sm4.decryptStr(valueStr);
                            entity.set(field, decryptStr);
                        } catch (Exception e) {
                            log.warn("数据解密失败：" + valueStr, e);
                        }
                    });
                }
