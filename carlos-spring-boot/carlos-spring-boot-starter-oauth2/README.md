...
- ✅ JWT Token支持
- ✅ 多种授权模式（授权码、客户端凭证、刷新令牌）
- ✅ 自定义Token增强（用户信息、租户、部门、角色等）
- ✅ 灵活的客户端配置
- ✅ 基于注解的方法级权限控制
- ✅ 与carlos-spring-boot-starter-core的用户上下文集成
    ...
    clients:
      - client-id: my-client
        ...
        authorization-grant-types:
          - authorization_code
          - refresh_token
          # - password  # Spring Authorization Server 1.x 已移除支持
        redirect-uris:
...
## Spring Boot 3.x 重要变更

### Password 授权类型已移除

⚠️ **重要提示**：Spring Authorization Server 1.x（配合 Spring Boot 3.x）已完全移除对 `password` 授权类型的支持。

**原因**：

- Password 授权类型存在安全隐患（客户端需要直接接触用户密码）
- OAuth 2.0 安全最佳实践建议使用更安全的授权码模式
- Spring Security 团队决定不再支持此授权类型

**替代方案**：

1. **推荐：Authorization Code + PKCE**（用于公共客户端）
   ```yaml
   authorization-grant-types:
     - authorization_code
...
3. **自定义授权类型扩展**（如需保留类似密码登录的体验）
    - 实现自定义 `AuthenticationConverter`
    - 实现自定义 `AuthenticationProvider`

### Spring Security 6.x 变更

本模块基于 Spring Security 6.x，主要变更：

- 使用 `SecurityFilterChain` Bean 替代 `WebSecurityConfigurerAdapter`
- 请求匹配使用 `requestMatchers()` 替代 `antMatchers()`
- 方法安全使用 `@EnableMethodSecurity` 替代 `@EnableGlobalMethodSecurity`
- 新的 Lambda DSL 配置风格
...
6. **授权类型**：Spring Boot 3.x 版本不支持 password 授权类型

## 与其他模块集成

### 与carlos-spring-boot-starter-core集成

OAuth2模块与carlos-spring-boot-starter-core的认证体系完全集成：

- 使用`LoginUserInfo`作为用户信息载体
- 使用`UserContext`作为用户上下文
- 支持租户、部门、角色等多租户架构

### 与carlos-spring-boot-starter-redis集成
