...
import com.carlos.core.constant.basic.HttpConstants;
...
public class WebSocketFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (!"ws".equals(scheme) && !"wss".equals(scheme)) {
            ...
            URI wsRequestUrl = UriComponentsBuilder.fromUri(requestUrl).scheme(wsScheme).port(
                    requestUrl.getPort() + HttpConstants.WS_PORT).build().toUri();
