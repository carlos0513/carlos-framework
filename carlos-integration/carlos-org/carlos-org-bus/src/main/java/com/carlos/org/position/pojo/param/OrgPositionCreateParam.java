...
@Schema(description = "岗位表新增参数")
public class OrgPositionCreateParam {
    @NotBlank(message = "岗位编码，租户内唯一，如：JAVA_DEV_001不能为空")
    @Schema(description = "岗位编码，租户内唯一，如：JAVA_DEV_001")
    private String positionCode;
    @NotBlank(message = "岗位名称，如：Java开发工程师不能为空")
    ...
    @NotNull(message = "岗位类型：1标准岗（有编制），2虚拟岗（项目制），3兼职岗不能为空")
    @Schema(description = "岗位类型：1标准岗（有编制），2虚拟岗（项目制），3兼职岗")
    private Integer positionType;
    @NotNull(message = "岗位性质：1全职，2兼职，3实习，4外包不能为空")
    ...
    @NotNull(message = "当前在岗人数（冗余，实时计算）不能为空")
    @Schema(description = "当前在岗人数（冗余，实时计算）")
    private Integer currentCount;
    @NotNull(message = "空缺人数（冗余，实时计算）不能为空")
    ...
    @NotNull(message = "状态：0禁用，1启用，2冻结（暂停招聘）不能为空")
