...
import com.yunjin.boot.util.ResponseUtil;
import com.yunjin.core.auth.AuthConstant;
...
import com.yunjin.org.config.MenuApiMappingProperties;
import com.yunjin.org.config.OrgConstant;
import com.yunjin.org.config.OrgProperties;
import com.yunjin.org.config.RandoGemNumberGnerator;
import com.yunjin.org.login.LoginThreadLocal;
import com.yunjin.org.login.PasswordConvertUtil;
import com.yunjin.org.login.ThridLogin;
...
import com.yunjin.org.login.pojo.enums.ThirdLoginType;
...
public class LoginServiceImpl implements LoginService {
    ...
    private LoginSuccessVO doLogin(UserDTO user, LoginParam loginParam) {
        ...
        if (CollUtil.isNotEmpty(departmentId) && departmentId.size() == 1) {
            ...
            ResourceGroupService resourceGroupService = SpringUtil.getBean(ResourceGroupService.class);
            List<ResourceGroupDTO> groups = resourceGroupService.listByRoleIds(Collections.singleton(userDepartmentDTO.getRoleId()));
            user.setResourceGroup(groups);
        }
        // 获取部门信息
    ...
    @Override
    public LoginSuccessVO rztLogin(RztLoginParam loginParam) {
        log.info("rzt login param:{}", loginParam);
        ...
        // try {
        //     userInfo = RztUtil.getUserInfo(userId);
        //     log.info("Get rzt userinfo success: userinfo:{}", userInfo);
        // } catch (Exception e) {
        //     log.error("Get rzt userinfo failed: userid:{}", userId, e);
        ...
        login.setLoginType(LoginType.RZT);
        return doLogin(user, login);
    }


    @Override
    public LoginSuccessVO bigLinkAgeLogin(BigLinkAgeLoginParam loginParam) {
        log.info("big-linkage login param:{}", loginParam);
        String code = loginParam.getUserid();
        if (StrUtil.isBlank(code)) {
            throw new ServiceException("code不能为空");
        }
        ...
        LoginParam login = new LoginParam();
        login.setAccount(mobile);
        login.setLoginType(LoginType.BIG_LINKAGE);
        return doLogin(user, login);
    }

    @Override
    public LoginSuccessVO eventLogin(EventLoginParam loginParam) {
        log.info("event login param:{}", loginParam);
        ...
        LoginParam login = new LoginParam();
        login.setAccount(mobile);
        login.setLoginType(LoginType.EVENT);
        return doLogin(user, login);
    }

    @Override
    public LoginSuccessVO dingtalkLogin(DingtalkLoginParam loginParam) {
        log.info("dingtalk login param:{}", loginParam);
        String code = loginParam.getCode();
        if (StrUtil.isBlank(code)) {
            throw new ServiceException("code不能为空");
        }
        OapiV2UserGetResponse result = null;
        OapiV2UserGetResponse.UserGetResponse userInfo = null;
        try {
        ...
        LoginParam login = new LoginParam();
        login.setAccount(mobile);
        login.setLoginType(LoginType.DINGTALK);
        return doLogin(user, login);
    }
...
//            设置用户部门信息 刷新缓存
    ...
    @Override
    public LoginSuccessVO tfLoginLogin(TfLoginLoginParam loginParam) {
        log.info("tf-oauth login param:{}", loginParam);
        String code = loginParam.getCode();
        if (StrUtil.isBlank(code)) {
            throw new ServiceException("code不能为空");
        }
