...
import com.yunjin.form.api.ApiFormData;
...
import com.yunjin.org.login.PasswordConvertUtil;
...
import com.yunjin.org.pojo.excel.UserExcel;
import com.yunjin.org.pojo.excel.UserPageExcel;
import com.yunjin.org.pojo.excel.UserRegionInitExcel;
...
import com.yunjin.task.api.ApiTaskInfo;
import com.yunjin.task.pojo.ao.TaskAO;
import com.yunjin.util.PasswordUtil;
import com.yunjin.util.poi.ExcelUtils;
...
/*        List<UserDepartmentDTO> existDepts = userDepartmentService.getByUserId(user.getId());
    ...
    /**
     * 组织机构调整前检查用户下是否存在任务和表单
     *
     * @param userId    用户id
     * @param deptCodes 变更前的用户部门ids
     */
    private void checkUserTaskAndForm(String userId, Set<String> deptCodes) {

        // 获取用户机构变更前是否有创建的任务
        ApiTaskInfo apiTaskInfo = SpringUtil.getBean(ApiTaskInfo.class);
        Result<List<TaskAO>> tasks = apiTaskInfo.getTaskByCreaterAndDeptCodes(userId, deptCodes);
        // 获取用户机构变更前是否有创建的表单
        ApiForm apiForm = SpringUtil.getBean(ApiForm.class);
        Result<List<FormTableBaseInfoAO>> tables = apiForm.getTableByCreaterAndDeptCodes(userId, deptCodes);

        // 检测本账号+本机构下是否有任务和报表
        if (tasks.getData() != null && tables.getData() != null) {
            if (!tasks.getData().isEmpty() && !tables.getData().isEmpty()) {
                throw new ServiceException("本账号有待办任务和已有准入报表，请先做移交，谢谢！");
            } else if (!tasks.getData().isEmpty()) {
                throw new ServiceException("本账号有待办任务，请先做任务移交，谢谢！");
            } else if (!tables.getData().isEmpty()) {
                throw new ServiceException("本账号已有准入报表，请先做报表移交，谢谢！");
            }
        }
    }
    ...
    @Override
    public List<UserDeptRoleAO> getUserByDeptAndRoleV2(ApiWarehouseDataExportParam param) {

        ApiFormData apiFormData = SpringUtil.getBean(ApiFormData.class);
        Result<HashSet<String>> result = apiFormData.getDataFormDept(param);
        if (result == null || !result.getSuccess()) {
            log.error("远程服务调用失败！异常信息：" + result);
            throw new ServiceException("远程服务调用失败！异常信息：" + result.getMessage());
        }
        log.info("远程服务调用返回信息：{}", result.getData());
        HashSet<String> deptCodes = result.getData();
        String configValue = null;
        ApiSystemConfig apiSystemConfig = SpringUtil.getBean(ApiSystemConfig.class);
        Result<SysConfigAO> sysConfigAO = apiSystemConfig.getByCode("department-sub-leader");
        if (Boolean.TRUE.equals(sysConfigAO.getSuccess()) && (sysConfigAO.getData() != null)) {
            configValue = sysConfigAO.getData().getConfigValue();
            if (StringUtils.isBlank(configValue)) {
                log.error("分管领导配置信息查询失败：" + configValue);
                throw new ServiceException("分管领导配置信息查询失败：" + configValue);
            }
        }

        ApiUser api = SpringUtil.getBean(ApiUser.class);
        ApiUserDeptRoleParam roleParam = new ApiUserDeptRoleParam();
        if (StringUtils.isNotBlank(configValue)) {
            roleParam.setRoleIds(Collections.singleton(configValue));
        }
        if (CollUtil.isNotEmpty(deptCodes)) {
            roleParam.setDeptCodes(deptCodes);
        }

        // 通过机构和角色获取用户
        Result<List<UserDeptRoleAO>> roleResult = api.getUserByDeptAndRole(roleParam);
        if (!roleResult.getSuccess()) {
            log.error("Api request failed, message: {}, detail message:{}", roleResult.getMessage(), roleResult.getStack());
            throw new ServiceException(roleResult.getMessage());
        }
    ...
    public UserDTO getCurrentUser(MenuType menuType) {
        ...
        // TODO 获取角色资源，目前仅处理了指标资源
        List<RoleResourceGroupDTO> roleResourceGroup = roleResourceGroupRefService.getByRoleIds(roleIds);
        user.setRoleResources(roleResourceGroup);
        ...
        // 获取当前用户所有的角色权限组
        List<ResourceGroupDTO> resourceGroups = resourceGroupService.listByRoleIds(roleIds);
        user.setResourceGroup(resourceGroups);
        return user;
    }
    ...
    @Override
    public void exportLog(ParamIdSet<Serializable> param, HttpServletRequest request, HttpServletResponse response) {
        ExcelUtils<User> excelUtils = new ExcelUtils(User.class);
        if (CollectionUtils.isEmpty(param.getIds())) {
            throw new ServiceException("请选择需要导出的日志！");
        }
        List<User> sysLogs = this.userManager.getBaseMapper().selectBatchIds(param.getIds());
        if (CollectionUtils.isEmpty(sysLogs)) {
            log.debug("日志log列表为，sysLogs:{}", sysLogs);
            throw new ServiceException("查询列表为空，导出失败！");
        }
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMddHHmmss");
        String datetime = dateFormat.format(new Date());
        // excelUtils.init(sysLogs, "操作日志" + datetime, "操作日志记录", Excel.Type.EXPORT);
    ...
    public String getDefaultPwd(String id) {
        ...
        // 默认密码进行base64加密
        String defaultPassword = PasswordUtil.randomPassword(12);
