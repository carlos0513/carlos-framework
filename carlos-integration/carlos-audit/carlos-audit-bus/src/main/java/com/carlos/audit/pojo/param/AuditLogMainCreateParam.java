...
/**
 * <p>
 * 审计日志宽主表（合并数据变更、技术上下文、标签、附件，保留7天热数据） 新增参数封装
 * </p>
 *
 * @author Carlos
...
@Schema(description = "审计日志宽主表（合并数据变更、技术上下文、标签、附件，保留7天热数据）新增参数")
public class AuditLogMainCreateParam {
    @NotNull(message = "服务器时间，毫秒精度，CK分区键不能为空")
    @Schema(description = "服务器时间，毫秒精度，CK分区键")
    private LocalDateTime serverTime;
    @NotNull(message = "事件日期，冗余字段，方便分区不能为空")
    ...
    @NotBlank(message = "状态：SUCCESS-成功/FAIL-失败/PENDING-处理中/TIMEOUT-超时/PARTIAL_SUCCESS-部分成功不能为空")
    ...
    @NotBlank(message = "操作描述，人工可读的操作说明不能为空")
    ...
    @NotNull(message = "是否有数据变更详情：0-否/1-是不能为空")
    ...
    @NotNull(message = "旧数据是否压缩：0-否/1-是不能为空")
    @Schema(description = "旧数据是否压缩：0-否/1-是")
    private Boolean oldDataCompressed;
    @NotNull(message = "新数据是否压缩：0-否/1-是不能为空")
