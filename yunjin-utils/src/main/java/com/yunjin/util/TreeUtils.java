...
import com.yunjin.util.constants.BaseConstants;
import com.yunjin.util.exception.YJBaseException;
...
public class TreeUtils {
    ...
    /**
     * 构建树结构 存在默认参数 详见BaseConstants.Entity IdName = ID | FIdName = PARENT_ID | childrenName = CHILDREN | topNode = TOP_NODE | killScattered = false |
     * killNoneChild = true
     *
     * @param list 组装列表
     * @return 树结构列表
    ...
    public static <T> List<T> buildTree(List<T> list) {
        return buildTree(list, BaseConstants.Entity.ID.getCode(), BaseConstants.Entity.PARENT_ID.getCode(),
                BaseConstants.Entity.CHILDREN.getCode(), BaseConstants.TOP_ID, false, true);
    }

    /**
     * 构建树结构 存在默认参数 详见BaseConstants.Entity IdName = ID | FIdName = PARENT_ID | childrenName = CHILDREN | topNode = TOP_NODE | killNoneChild = true
     *
     * @param list          组装列表
    ...
    public static <T> List<T> buildTree(List<T> list, boolean killScattered) {
        return buildTree(list, BaseConstants.Entity.ID.getCode(), BaseConstants.Entity.PARENT_ID.getCode(),
                BaseConstants.Entity.CHILDREN.getCode(), BaseConstants.TOP_ID, killScattered, true);
    }

    /**
     * 构建树结构 存在默认参数 详见BaseConstants.Entity IdName = ID | FIdName = PARENT_ID | childrenName = CHILDREN | topNode = TOP_NODE
     *
     * @param list          组装列表
     * @param killScattered 是否移除无法追溯到顶级节点 (true 是 | false 否)
    ...
    public static <T> List<T> buildTree(List<T> list, boolean killScattered, boolean killNoneChild) {
        return buildTree(list, BaseConstants.Entity.ID.getCode(), BaseConstants.Entity.PARENT_ID.getCode(),
                BaseConstants.Entity.CHILDREN.getCode(), BaseConstants.TOP_ID, killScattered, killNoneChild);
    }

    /**
     * 构建树结构 存在默认参数 详见BaseConstants.Entity IdName = ID | FIdName = PARENT_ID | childrenName = CHILDREN | killNoneChild = true
     *
     * @param list          组装列表
     * @param topNode       顶级节点
     * @param killScattered 是否移除无法追溯到顶级节点 (true 是 | false 否)
    ...
    public static <T> List<T> buildTree(List<T> list, Serializable topNode, boolean killScattered) {
        return buildTree(list, BaseConstants.Entity.ID.getCode(), BaseConstants.Entity.PARENT_ID.getCode(),
                BaseConstants.Entity.CHILDREN.getCode(), topNode, killScattered, true);
    }
     ...
     * @return 树结构列表
     */
    public static <T> List<T> buildTree(List<T> list, String IdName, String FIdName, String childrenName,
                                        Serializable topNode, boolean killScattered, boolean killNoneChild) {
        ...
        try {
            if (CollUtil.isNotEmpty(list)) {
                ...
                for (T vo : list) {
                    ...
                    if (ObjUtil.equals(Id.get(vo), topNode)) {
                        hasTopNode = true;
                        top = vo;
                        list.remove(vo);
                        break;
        ...
        if (hasTopNode && ObjUtil.isNotNull(top)) {
            List<T> topList = new ArrayList<>();
            try {
                Field children = depthRecursive(top, checkAttribute(top, childrenName, 0))
                        .getDeclaredField(childrenName);
    ...
    private static <T> List<T> getChildList(List<T> list, T t, String IdName, String FIdName) {
        ...
        try {
            while (it.hasNext()) {
                ...
                FId.setAccessible(true);
                if (ObjUtil.isNotNull((Long) FId.get(n))
                        && ((Long) FId.get(n)).longValue() == ((Long) Id.get(t)).longValue()) {
                    tList.add(n);
                }
            }
        } catch (NoSuchFieldException | IllegalAccessException e) {
    ...
    /**
     * 获取树结构叶子节点集合 存在默认参数 详见BaseConstants.Entity childrenName = CHILDREN
     *
     * @param list 组装列表
     * @return 叶子节点集合
     */
    ...
    /**
     * 获取树结构叶子节点集合 存在默认参数 详见BaseConstants.Entity
     *
     * @param list         组装列表
     * @param childrenName children字段名称
     * @return 叶子节点集合
